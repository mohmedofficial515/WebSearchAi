...
export function LoginCard({ taskId, goal, onSuggestion, onContinue, onEnd }: LoginCardProps) {
      ...
      <div className={`rounded-2xl border ${borderColor} bg-white dark:bg-slate-900 shadow-sm overflow-hidden`}>
        ...
        <div className="flex items-center gap-3 px-5 py-3 border-b border-slate-100 dark:border-slate-800">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-400">تسجيل دخول</p>
          </div>
          {isDone && (
          ...
          {isDone && !success && (
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-rose-600 dark:text-rose-400">
                <span>❌</span>
                <span className="text-sm font-medium">فشل تسجيل الدخول</span>
              </div>
              {(stream.error || stream.summaryAr) && (
